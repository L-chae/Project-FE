...
export function useSetupForm() {
  ...
  const [selected, setSelected] = useState(""); // ✅ 관심 분야 단일 값(string). 미선택이면 ""
  ...
  // ✅ 단일 선택: 클릭 시 선택 / 같은 값 다시 클릭 시 해제(원하면 해제 금지로 바꿀 수 있음)
  ...
  const handleRegister = async (options = {}) => {
    ...
    // ✅ preference는 단일 string 또는 null
    const preferenceValue =
      overridePreference ?? (selected ? selected : null);
